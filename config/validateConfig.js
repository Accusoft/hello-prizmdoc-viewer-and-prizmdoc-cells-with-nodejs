...
function notConfigured() {
  return (config.pasBaseUrl === PRIZMDOC_CLOUD_PAS_BASE_URL || config.cellsServerBaseUrl === PRIZMDOC_CLOUD_CELLS_SERVER_BASE_URL) && config.apiKey === PLACEHOLDER_API_KEY;
}

async function validateConfig() {
  if (notConfigured()) {
    console.error(`ERROR: You need to edit ${CONFIG_FILENAME} to configure your server connection. If you're just getting started, you can use our hosted PrizmDoc Cloud service; all you need to do is configure your API key. Visit https://cloud.accusoft.com to sign up for an account and get an API key at no cost. See the README.md file for more information.`);
    process.exit(0);
  }
